// Decompiled with JetBrains decompiler
// Type: org.jfree.base.modules.ModuleInitializeException
// Assembly: SmartDashboard, Version=0.0.0.0, Culture=neutral, PublicKeyToken=null
// MVID: 9F306A9B-2142-4463-973F-A516156E3EE9
// Assembly location: C:\Users\Matan\Documents\Repositories\SmartDashboard.NET\DLLs\SmartDashboard.dll

using IKVM.Attributes;
using java.lang;
using org.jfree.util;
using System;
using System.Runtime.CompilerServices;
using System.Runtime.InteropServices;
using System.Runtime.Serialization;
using System.Security.Permissions;

namespace org.jfree.@base.modules
{
  [Serializable]
  public class ModuleInitializeException : StackableException
  {
    [LineNumberTable(new byte[] {(byte) 32, (byte) 105})]
    [MethodImpl(MethodImplOptions.NoInlining)]
    public ModuleInitializeException(string s)
      : base(s)
    {
    }

    [LineNumberTable(new byte[] {(byte) 22, (byte) 106})]
    [MethodImpl(MethodImplOptions.NoInlining)]
    public ModuleInitializeException(string s, Exception e)
      : base(s, e)
    {
    }

    [LineNumberTable(new byte[] {(byte) 10, (byte) 136})]
    [MethodImpl(MethodImplOptions.NoInlining)]
    public ModuleInitializeException()
    {
    }

    [HideFromJava]
    [PermissionSet(SecurityAction.Demand, XML = "<PermissionSet class=\"System.Security.PermissionSet\"\r\nversion=\"1\">\r\n<IPermission class=\"System.Security.Permissions.SecurityPermission, mscorlib, Version=2.0.0.0, Culture=neutral, PublicKeyToken=b77a5c561934e089\"\r\nversion=\"1\"\r\nFlags=\"SerializationFormatter\"/>\r\n</PermissionSet>\r\n")]
    protected ModuleInitializeException([In] SerializationInfo obj0, [In] StreamingContext obj1)
      : base(obj0, obj1)
    {
    }
  }
}
