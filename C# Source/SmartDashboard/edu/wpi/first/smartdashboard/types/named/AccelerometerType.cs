...
// Type: edu.wpi.first.smartdashboard.types.named.AccelerometerType
...
{
  public class AccelerometerType : NamedDataType
  {
    public const string LABEL = "Accelerometer";

    [HideFromJava]
    static AccelerometerType()
    {
    ...
    private AccelerometerType()
      : base("Accelerometer", (Class) ClassLiteral<SingleNumberDisplay>.Value, new DataType[0])
    {
    }
    ...
    {
      if (NamedDataType.get("Accelerometer") != null)
        return NamedDataType.get("Accelerometer");
      else
        return (NamedDataType) new AccelerometerType();
    }
  }
}
